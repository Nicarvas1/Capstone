...
def transcribir_audio(audio_bytes):
    """Transcribe audio bytes a texto usando Whisper (sin ffmpeg)."""
    ...
    try:
        ...
        modelo_w = cargar_whisper()
        result = modelo_w.transcribe(audio_data, language="es", fp16=False)
...
if "audio_transcripcion" not in st.session_state:
    st.session_state.audio_transcripcion = ""
if "audio_procesado" not in st.session_state:
    st.session_state.audio_procesado = False
if "prompt_desde_audio" not in st.session_state:
    st.session_state.prompt_desde_audio = ""
...
# ==========================================
# 5. AUDIO (MICRÓFONO + WHISPER)
# ==========================================
with st.expander("🎤 Usar micrófono en vez de escribir", expanded=False):
    st.caption("Graba tu mensaje de voz y se transcribirá automáticamente al chat.")
    audio_input = st.audio_input("Presiona para grabar", key="mic_input")

    if audio_input is not None and not st.session_state.audio_procesado:
        with st.spinner("🎧 Transcribiendo audio con Whisper small..."):
            try:
                texto_transcrito = transcribir_audio(audio_input)
                st.session_state.audio_transcripcion = texto_transcrito
                st.session_state.audio_procesado = True
            except Exception as e:
                st.error(f"Error al transcribir: {e}")

    if st.session_state.audio_transcripcion:
        st.success("✅ Transcripción lista:")
        texto_mostrar = st.session_state.audio_transcripcion
        st.info(f'📝 "{texto_mostrar}"')
        col_env, col_des = st.columns([2, 1])
        with col_env:
            if st.button("📨 Enviar transcripción al chat", type="primary", use_container_width=True):
                st.session_state.prompt_desde_audio = st.session_state.audio_transcripcion
                st.session_state.audio_transcripcion = ""
                st.session_state.audio_procesado = False
                st.rerun()
        with col_des:
...
# ==========================================
# 6. INTERFAZ DEL CHAT (ARQUITECTURA DE SUPERVISOR)
# ==========================================
...
# Inyectar transcripción de audio como si fuera un prompt escrito
if "prompt_desde_audio" in st.session_state and st.session_state.prompt_desde_audio:
    prompt = st.session_state.prompt_desde_audio
    st.session_state.prompt_desde_audio = ""
